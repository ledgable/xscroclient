...
#					appVars.token = container_.find(argid)
#
#				elif (mode == "owner"):
#					appVars.container = container_
...
#		return FunctionResponse(HTTP_OK, TYPE_JSON, {})
	...
	@endpoint(1, False, True, None, "put", "^/api/transfer/(?P<transid>[0-9a-f][^-&*/\%]*)/(?P<volume>[0-9.][^-&*/\%]*)/(?P<recipientid>[0-9a-z][^-&*/\%]*)/(?P<price>[0-9.][^-&*/\%]*)", "Buy a new token")
	def transferValue(self, postData=None, appVars=None, transid=None, volume=0.0, recipientid=None, price=0.0):
		...
		if (success_):
			...
			success_, uid_, status = XscroController.transferValue(self, chainid_, wallet_, recipientid, transid, volume, price, "Token Transferred", appVars.ip_address, None)
	...
	# /api/destroy/<transid>/<volume>
	# transfer a token to receipient 0 - effectively destorying it
	
	@endpoint(1, False, True, None, "delete", "^/api/destroy/(?P<transid>[0-9a-f][^-&*/\%]*)/(?P<volume>[0-9.][^-&*/\%]*)", "Destroy a token")
	def destroyValue(self, postData=None, appVars=None, tokenid=None, transid=None, volume=0.0):
		...
		if (success_):
			success_, uid_, status = XscroController.destroyValue(self, chainid_, wallet_, transid, volume, "Token Destroyed", appVars.ip_address, None)
...
#		return FunctionResponse(HTTP_OK, TYPE_JSON, {})
	...
	def getOpenTransactions(self, postData=None, appVars=None, chainid="", mode="open"):
		...
		if (chainid_ != None):
			...
			if (chainid_ in xscro_.containers.keys()):
				...
				if (mode == "open"):
					transactions_ = list(container_.pending.values())
		
		return FunctionResponse(HTTP_OK, TYPE_JSON, {"status":STATUS_OK, "chainid":chainid_, "transactions":transactions_})
	...
	def ackTransaction(self, postData=None, appVars=None, token=None, code=0):
		...
		if (success_) and (token != None):
			...
			success_, response_ = XscroController.ackTransaction(self, chainid_, wallet_, token, code)
	...
	# /api/<chainid>/balance/<owner>/<argid>
	# get balance for appropriate request

	@endpoint(1, False, True, None, "get", "^/api/(?P<chainid>[0-9a-f][^-&*/\%]*)/balance/(?P<mode>(owner))/(?P<argid>[0-9a-z][^-&*/\%]*)", "Get value of token or owner")
		...
		if (chainid != None):
			...
			if (argid != None):
				...
				if (chainid_ in xscro_.containers.keys()):
					...
					wallet_ = container_.walletFor(argid)
	...
	# /api/<chainid>/transactions/<owner>/<argid>
	# get transactions for appropriate request (does not include pending)

	@endpoint(1, False, True, None, "get", "^/api/(?P<chainid>[0-9a-f][^-&*/\%]*)/transactions/(?P<mode>(owner))/(?P<argid>[0-9a-z][^-&*/\%]*)", "Get transactions for owner")
	def transactionsFor(self, postData=None, appVars=None, chainid="", mode="token", argid=None):

		transactions_ = XscroController.allTransactionsForWallet(self, chainid, argid)
		
		return FunctionResponse(HTTP_OK, TYPE_JSON, {"status":STATUS_OK, "ownerid":argid, "transactions":transactions_})
