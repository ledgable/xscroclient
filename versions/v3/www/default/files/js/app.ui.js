...
App.UI.Core = {
	...
	doVarSubstitution : function(data, line) {
		...
		while((m=r.exec(line))!=null&&m.length>1&&m[1]!=''){
			...
			if (posrepl > 0) { // sequence start..
				if (sequence[posrepl-1] != ".") {
					isarray = true;
			...
			if (!isarray) {			
					...
					switch (datatype) {
						...
						case 'fn': {
							...
							switch (formatting) {
								...
								case "bignumber": {
									var number = Number(dataItem);
									if (number > 1000000000) {
										var count = number / 1000000000;
										var dp = parseInt(count * 100 - (parseInt(count) * 100));
										dataItem = "" + parseInt(count) + "." + dp + "B";
									} else if (number > 1000000) {
										var count = number / 1000000;
										var dp = parseInt(count * 100 - (parseInt(count) * 100));
										dataItem = "" + parseInt(count) + "." + dp + "M";
									} else {
										dataItem = Number(dataItem).toLocaleString('en');
									}
									break;
								}
								...
								case "currency": {
									dataItem = Object.toCurrency(dataItem, 2);
									break;
								}
								case "currency3": {
									dataItem = Object.toCurrency(dataItem, 3);
									break;
								}
				...
				if ((dataItem != null) && (dataItem.length>0)) {
					...
					arraycontent = arraycontent.replace(/&amp;&amp;/gi, '&&');
					arraycontent = arraycontent.replace(/&amp;/gi, '&');
					arraycontent = arraycontent.replace(/&perc;/gi, '%');
					arraycontent = arraycontent.replace(/&lt;/gi, '<');
					arraycontent = arraycontent.replace(/&gt;/gi, '>');
					...
					for (var counter = 0; counter< dataItem.length; counter++) {
						...
						if ((dataItem[counter] != null) && (dataItem[counter].uid != 'undefined') && (dataItem[counter].uid != null)) {
							oHtml += '<li data-id="' + dataItem[counter].uid + '"' + subst + '</li>';
						} else {
							oHtml += '<li ' + subst + '</li>';
						}
...
App.UI.List = {
	...
	refresh : function(target) {
				...
				$('body').find('[data-component="list"]').each(function() {
					...
					$(target.object).find('[data-component="list"]').each(function() {
																		  App.UI.List.__createList(this);
																		  });
	...
	__createList : function(target) {
		...
		if ((datasource != null) && (datasource != '')) {
			...
			if (datasource.startsWith('arr:')) {			
				...
				App.UI.DataServices.getData(datasource, target, App.UI.List.__processJsonData, usecache);
			}
...
App.UI.Repeater = {
	...
	__preinit : function() {
		...
		$('*').bind('page-refresh window-refresh data-refresh', function(object, data) {
	...
	refresh : function(target) {
		...
		if (target == null) {
				...
				if (target.object != null && target.object != 'undefined') {
					...
					$(target.object).find('[data-component="repeater"]').each(function() {
																	App.UI.Repeater.__createList(this);
																			  });
...
App.UI.DatePicker = {
	...
	__preinit : function() {
		if (this.hasInit) return;
		this.hasInit = true;
		$('*').bind('data-refresh page-refresh window-refresh', function(object, data) {
				App.UI.DatePicker.init(data);
			});
	},

	init : function(parent) {

		$('.datepicker.past').each(function() {
							   var options = App.UI.DatePicker.config.past;
							   $(this).datepicker(options);
							   });

		$('.datepicker.sixteenplus').each(function() {
									  var options = App.UI.DatePicker.config.sixteenplus;
									  $(this).datepicker(options);
									  });

		$('.datepicker.pastbusiness').each(function() {
									   var options = App.UI.DatePicker.config.pastbusiness;
									   $(this).datepicker(options);
									   });

		$('.datepicker.def').each(function() {
							  var options = App.UI.DatePicker.config.def;
							  $(this).datepicker(options);
							  });

		$('.datepicker.present').each(function() {
								  var options = App.UI.DatePicker.config.present;
								  $(this).datepicker(options);
								  });
...
App.UI.LoadFile = {
	...
	__preinit : function() {
		...
		$(document).delegate('input[type="file"]', 'change', function(oEvent) {
							 App.UI.LoadFile.__loadFile(oEvent.currentTarget);
							 });

	},

	__loadFile : function(target) {

		if (target.files && target.files[0]) {

			var _file = target.files[0];
			$(target).attr('data-filename', _file.name);

			var _reader = new FileReader();
			_reader.onload = function (e) {
				var _data = e.target.result;
				$(target).attr('data-loadedcontent', _data);
				console.info(_data);
			};
...
App.UI.Stepshow = {
	...
	__preinit : function() {
		...
		$(document).delegate('ul.pips li.pip', 'click', function(oEvent) {
	...
	pipClicked : function(oEvent) {
		...
		var target = $(oEvent.currentTarget);
		var parent = $(target).parent().closest('[data-component="stepshow"]');
				
		if ((parent == null) || (parent == 'undefined')) return;

		var slideindex = $(target).index();
		var slides = $('> .steps > li, form > .steps > li', parent);
		...
		$('*').triggerHandler('data-refresh', "#" + target.id);
		$('*').triggerHandler('tab-changed', slides[slideindex]);
