...
Object.toCurrency = function(o, dp, showcomma) {
	var i = parseFloat(o);
	if (isNaN(i)) i = 0.00;
	var minus = (i < 0) ? '-' : '';
	i = Math.abs(i).toFixed(dp);
	var showComma = (showcomma != null && showcomma != 'undefined') ? showcomma : true;
	var s = showComma ? Number(i).toLocaleString('en') : new String(i);
	switch(dp) {
		case 0: {
			break;
		}
...
App.Core.Nav = {
	...
	__preinit : function() {
		...
		$(document).delegate(".pagenav", 'click', function(oEvent) { App.Core.Nav.target(oEvent); return true; });
		
		$('[data-seo]').delegate('click', function(oEvent) { App.Core.Nav.informSeo(oEvent); });
		...
		setInterval(function(){
					...
					if ((newhash == null) || (newhash == 'undefined') || (newhash == '')) newhash = '/';
					
					if (newhash != hash) {
					
						var index_ = App.Core.Nav.hashHistory.indexOf(newhash.substring(1));
					
						if (index_ == (App.Core.Nav.hashHistory.length-1)) {
					
							// same page
							App.Core.Nav.__goto(newhash.substring(1), null, false);
					
						} else if (index == -1) {
					
							App.Core.Nav.hashHistory.push(newhash);
							if (newhash.substring(1) != App.Core.Nav.currentUrl) {
	...
	target : function(oEvent) {
		...
		if ((url != null) && (url != 'undefined')) {
			...
			if (oEvent.currentTarget != null) {
				...
				useEvent = {'type':oEvent.type, 'actualTarget':oEvent };
				
			}
			
			this.__goto(encodeURI(url), useEvent, false);
	...
	informSeo : function(oEvent) {
		...
		var seoUrl = App.Core.Nav.seoUrl(seoAttr);
	...
	goto : function(url, force) {
		if ((force=='undefined') || (force==null)) force=false;
		if (url=='') url = '/';
		this.__goto(url, null, force);
	},
	
	seoUrl : function(url) {
		
		if (url.startsWith('$http')) return {'url':url, 'accept':false};
	...
	__goto : function(url, oEvent, force) {
		...
		if (url[0]=='$') {
			...
			if (oEvent != null) {
				...
				if ((target == null) || (target=='undefined')) {
					target = $(oEvent.actualTarget).attr('data-target');
				}
			...
			var seoUrl = App.Core.Nav.seoUrl(url);
			
			if (seoUrl.accept) {
				window.location.hash = seoUrl.hash;
			}
			
			if ((url != App.Core.Nav.currentUrl) || force) {
				
				App.Core.Nav.currentUrl = url;

				var ajaxContainer = $('#mainbody');
...
App.Core.Application = {
	...
	__preinit : function(){
		...
		$(document).delegate('input', 'change', function(oEvent){ App.Core.Application.__handleEvent(oEvent, 'event'); });

		$(document).delegate('[data-event]:not(input):not(textarea):not(select):not([data-component="gmaps"])','click', function(oEvent){ App.Core.Application.__handleEvent(oEvent, 'event'); });
	...
	__handleEvent : function(oEvent, type, initiator, params) {
		...
		if (params != null && (params != 'undefined')) {
			if (oArgs.args == null || oArgs.args == 'undefined') {
				oArgs.args = {};
			}
			oArgs.args.file = params;
		}
	...
	__handleStatus : function(oData){
		...
		switch (oData.mode){
			...
			case 'notify' : {
				App.Notify.showMessage(oData.status, oData.message);
				break;
			}
		...
		if (oData.refresh != null && oData.refresh != 'undefined') {
		
			switch (oData.refresh){
				...
				case 'internal': {
					App.Core.redirect('internal', oData.url);
					break;
				}
					
				case 'redirect': {
					setTimeout(function() { App.Core.redirect('direct', oData.url); }, 2);
					break;
				}
