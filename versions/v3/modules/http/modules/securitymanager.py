...
class SecurityMgr(BaseClass):
	...
	def getDigestForUser(self, username, realm="basic"):
	
		permissions_ = None
	
		if (realm in self.realms_):
			permissions_ = self.realms_[realm]
	
		if (permissions_ != None):
			allusers_ = list(permissions_.keys())

			if (username in allusers_):
				userinfo_ = permissions_[username]
				return userinfo_.password

		return None

			
	def permissionsForUser(self, username, realm="basic"):
